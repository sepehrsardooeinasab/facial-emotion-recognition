...
import torch
from archs import MixedFeatureNetPT
...
class DDAMNet(nn.Module):
    def __init__(self, num_class=7,num_head=2, pretrained=False, pretrained_address=os.path.join('./weights/FERPlusNew/DDAMPT', "best_model.pth")):
        super(DDAMNet, self).__init__()

        net = MixedFeatureNetPT.MixedFeatureNet()
                
        if pretrained:
            net = torch.load(pretrained_address)       
